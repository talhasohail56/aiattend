...
import { MapPin, LogOut, Users, TrendingUp, Download, Plus, Edit, Trash2, AlertTriangle, CheckCircle, Clock, XCircle, BarChart3, PieChart, Key, Copy, Eye, EyeOff, CalendarClock } from 'lucide-react'
...
export default function AdminPage() {
  ...
  const handleCreateOverride = async () => {
    if (!overrideEmployee || !overrideDate || !overrideTime) return
    ...
    try {
      const response = await fetch('/api/admin/overrides', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: overrideEmployee.id,
          shiftDate: overrideDate, // Client sends YYYY-MM-DD
          newCheckInTime: overrideTime,
      ...
      setOverrideDate('')
      setOverrideTime('')
      alert('Schedule override created successfully')
    } catch (error: any) {
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setOverrideDate(new Date().toISOString().split('T')[0]) // Default today
                                setOverrideTime('21:00')
                                setOverrideOpen(true)
                              }}
                              className="bg-neutral-800 border-neutral-700 text-blue-500 hover:bg-neutral-700 hover:text-blue-400"
        ...
        <Dialog open={overrideOpen} onOpenChange={setOverrideOpen}>
          <DialogContent className="bg-neutral-900 border-neutral-800 text-neutral-200">
            <DialogHeader>
              ...
              <DialogDescription className="text-neutral-500">
                Adjust the start time for <span className="text-white font-medium">{overrideEmployee?.name}</span> for a specific day.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
              <div className="space-y-2">
                <Label className="text-neutral-200">Shift Date</Label>
                <Input
                  type="date"
                  value={overrideDate}
              ...
              <div className="space-y-2">
                <Label className="text-neutral-200">New Start Time</Label>
                <Input
                  type="time"
                  value={overrideTime}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setOverrideOpen(false)} className="border-neutral-700 text-neutral-400 hover:bg-neutral-800">
                Cancel
              </Button>
              <Button onClick={handleCreateOverride} className="bg-blue-600 hover:bg-blue-700 text-white">
                Save Override
              </Button>
            </DialogFooter>
          </DialogContent>
